...
import { AlertTriangle, Moon, Sun, Tag, Calendar, Settings, LogOut } from 'lucide-react';
...
const Sidebar = () => {
    ...
    const fetchProfile = async () => {
      ...
      if (session?.user) {
        ...
        setProfile({
          ...
          email,
          role: profileData?.role || session.user.user_metadata?.role || 'seller',
  ...
  const navLinks = [
    ...
    { to: "/map", label: 'Map View' },
    {
      to: "/warehouse",
      label: 'Warehouse',
      badge: overflowing.length > 0 ? (
        <span style={{ display: 'inline-flex', alignItems: 'center', gap: '4px' }}>
          {overflowing.length} <AlertTriangle size={12} style={{ color: 'white' }} />
        </span>
      ) : null
    },
                ...
                <div style={{ display: 'flex', flexDirection: 'column', gap: '6px', fontSize: '0.85rem' }}>
                  <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
                    ...
                    <span style={{ fontWeight: 'bold', color: 'var(--text-primary, #1e293b)' }}>
                      {profile.role ? (profile.role.charAt(0).toUpperCase() + profile.role.slice(1)) : 'Seller/Admin'}
                    </span>
                  </div>
                  {profile.memberSince && (
                    <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
